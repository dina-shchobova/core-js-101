...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (Object.keys(brackets).includes(str[i])) {
      if (arr.length && arr[arr.length - 1] === brackets[str[i]]) {
        arr.pop();
      } else {
        return false;
      }
...
function getCommonDirectoryPath(pathes) {
  const all = pathes.map((item) => item.split('/')).reduce((a, b) => a.concat(b));
  ...
  const newArr = res.filter((item, index, arr) => arr.indexOf(item) === index);
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < position.length; i += 1) {
    ...
    if (position[i][0] !== undefined && position[i][0] === position[i][1] && position[i][1] === position[i][2]) {
      return position[i][0];
    }
    // eslint-disable-next-line max-len
